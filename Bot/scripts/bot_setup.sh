...
mkdir -p bot_folder
mkdir -p bot_folder/restart_module
mkdir -p bot_folder/logger_system
mkdir -p bot_folder/keyboard_module
mkdir -p bot_folder/functions_module
mkdir -p bot_folder/db_module
mkdir -p bot_folder/config_module

# 1) restart_module
cat > bot_folder/restart_module/restart_bot.py << 'EOL'
import os
import sys
import time
import subprocess
...
# 2) logger_system
cat > bot_folder/logger_system/__init__.py << 'EOL'
import logging, sys
...
cat > bot_folder/logger_system/logger.py << 'EOL'
import logging, sys
...
# 3) keyboard_module
cat > bot_folder/keyboard_module/keyboard.py << 'EOL'
...
# 4) functions_module
cat > bot_folder/functions_module/config.py << 'EOL'
TEXTS_ROOT = './texts'
MEDIA_ROOT = './images'
EOL

cat > bot_folder/functions_module/functions.py << 'EOL'
...
# 5) db_module
cat > bot_folder/db_module/config.py << 'EOL'
DB_NAME = 'sql_db'
DB_TABLE_NAME = 'main_table'
EOL

cat > bot_folder/db_module/db.py << 'EOL'
import sqlite3
import json
...
# 6) config_module
touch bot_folder/config_module/.env
echo "ACCESS_TOKEN =" > bot_folder/config_module/.env

cat > bot_folder/config_module/config.py << 'EOL'
import os
